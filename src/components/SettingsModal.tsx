      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full overflow-hidden border border-slate-200">
        ...
        <div className="p-5 sm:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 font-medium text-xs cursor-pointer"
                  >
                    {(formData.operatorsList || DEFAULT_OPERATORS_LIST).map((op) => (
            ...
            {/* Quản lý danh sách người làm báo cáo */}
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5 text-emerald-600" />
                  Danh Sách Người Làm Báo Cáo ({(formData.operatorsList || DEFAULT_OPERATORS_LIST).length} người)
                </label>
                <button
                  type="button"
                  onClick={() => {
                    setFormData({
                      ...
                      defaultOperator: DEFAULT_OPERATORS_LIST[0],
                    });
                  }}
                  className="text-[10px] text-slate-500 hover:text-slate-700 flex items-center gap-1 underline font-medium"
                >
                  <RotateCcw className="w-3 h-3" />
                  Khôi phục 7 người gốc
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {(formData.operatorsList || DEFAULT_OPERATORS_LIST).map((op) => (
                  <span
                    key={op}
                    className="inline-flex items-center gap-1 bg-white border border-slate-300 text-slate-800 px-2 py-1 rounded-lg text-xs font-semibold shadow-2xs"
                  >
                    <span>{op}</span>
                    <button
                      type="button"
                      onClick={() => {
                        const currentList = formData.operatorsList || DEFAULT_OPERATORS_LIST;
                        if (currentList.length <= 1) return;
                        const filtered = currentList.filter((item) => item !== op);
                        setFormData({
                          ...formData,
                          operatorsList: filtered,
                          defaultOperator: formData.defaultOperator === op ? filtered[0] : formData.defaultOperator,
                        });
                      }}
              ...
              <div className="flex gap-2 pt-1">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      const clean = newOpInput.trim();
                      if (!clean) return;
                      const currentList = formData.operatorsList || DEFAULT_OPERATORS_LIST;
                      if (!currentList.includes(clean)) {
                        setFormData({
                          ...formData,
                          operatorsList: [...currentList, clean],
                        });
                        setNewOpInput('');
                      }
                    }
                  }}
                  className="flex-1 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-medium focus:outline-none focus:ring-1 focus:ring-emerald-500"
                />
                <button
                  type="button"
                  onClick={() => {
